  ...
  return (
    <div class="py-24 sm:py-32">
      <div class="max-w-3xl mx-auto px-6">
        {/* Header */}
        <div class="mb-16">
          <p class="text-zinc-600 dark:text-zinc-500 text-[13px] font-medium uppercase tracking-wider mb-4">Careers</p>
          <h1 class="text-4xl sm:text-5xl font-semibold text-zinc-900 dark:text-white tracking-tight mb-6">
            Working on Teploy
          </h1>
          <p class="text-zinc-700 dark:text-zinc-400 text-[16px] leading-relaxed max-w-xl">
                ...
                >
                  <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-3">
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                      <span class="px-2.5 py-1 bg-zinc-200/70 dark:bg-white/[0.06] text-zinc-700 dark:text-zinc-400 text-[11px] font-medium uppercase tracking-wider rounded-full">
                        {job.department}
                      </span>
                      <span class="text-zinc-700 dark:text-zinc-600 text-[13px]">{job.location}</span>
                      <span class="text-zinc-700">&middot;</span>
                      <span class="text-zinc-700 dark:text-zinc-600 text-[13px]">{job.type}</span>
                    </div>
                  </div>
                  <p class="text-zinc-600 dark:text-zinc-500 text-[14px]">{job.description}</p>
                </a>
              ))}
            </div>
          ) : (
            <div class="p-8 rounded-xl border border-zinc-200/70 dark:border-white/[0.06] bg-zinc-100/60 dark:bg-white/[0.02] text-center">
              <p class="text-zinc-700 dark:text-zinc-400 text-[15px] mb-4">We don't have any open positions right now.</p>
              <p class="text-zinc-600 dark:text-zinc-500 text-[14px]">
                Interested in working with us in the future? Send a note to{' '}
                <a href="mailto:contact@teploy.com" class="text-zinc-900 dark:text-white hover:text-zinc-800 dark:hover:text-zinc-300 underline underline-offset-2 transition-colors">
                  contact@teploy.com
                </a>
              </p>
            </div>
